...
execute as @a[nbt={Inventory:[{id:"minecraft:golden_helmet",Slot:103b},{id:"minecraft:golden_chestplate",Slot:102b},{id:"minecraft:golden_leggings",Slot:101b},{id:"minecraft:golden_boots",Slot:100b}]}] run effect give @s fire_resistance

execute as @e[type=area_effect_cloud] run data merge entity @s {Age:0}

execute as @e[type=armor_stand,tag=despawner] at @s run kill @e[type=area_effect_cloud,sort=nearest,limit=1,distance=0..5]
execute as @e[type=armor_stand,tag=lootbox_spawner] at @s run summon area_effect_cloud ~ ~ ~ {Duration:2147483647,Tags:["despawner"]}
kill @e[type=armor_stand,tag=spawnegg]
